...
// dismisses it.
import { BrowserWindow, screen, type Display, type WebPreferences } from 'electron';
import path from 'path';
import os from 'os';

import type { IdentifyOverlayPayload } from '../../types/client';
...
let onUserClose: (() => void) | null = null;
...
function normalizeBounds(
  candidate: Partial<OverlayBounds> | null | undefined,
...
export const Manager = {
  ...
  // webPreferences and can notify the socket layer when the user dismisses it.
  Configure({ webPreferences, onClose }: ConfigureOptions = {}): void {
  ...
  Show(Payload: IdentifyOverlayPayload = {}): void {
    ...
    const Data = {
      ...
      IPs: Payload && Array.isArray(Payload.IPs) ? Payload.IPs.map(String) : [],
    };
  ...
  // User dismissed the overlay (esc / click). Hide it and notify the socket
  // layer once so the server can clear identify state.
