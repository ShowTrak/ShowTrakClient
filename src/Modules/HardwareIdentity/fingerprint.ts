...
// in ./probes.ts so this logic stays directly unit-testable.

import os from 'node:os';
import { v5 as uuidv5 } from 'uuid';

/** The subset of an `os.networkInterfaces()` address entry this module reads. */
export interface MacCandidateAddress {
  mac?: string;
  internal?: boolean;
}

/** Shape of `os.networkInterfaces()`, loosened so tests can pass fixtures. */
export type NetworkInterfaceMap = Record<string, MacCandidateAddress[] | undefined>;
...
export function IsTrustworthyFirmwareId(value: unknown): boolean {
...
function IsVirtualMac(hex: string): boolean {
...
export function SelectPhysicalMacs(interfaces: NetworkInterfaceMap | null | undefined): string[] {
...
export function GetPhysicalMacs(): string[] {
  try {
...
export function DeriveUUID(source: string, evidence: string): string {
...
export function MacWitness(macs: string[]): string {
  return macs.join('|');
}

export function ParseMacWitness(witness: unknown): string[] {
